...
namespace SQLite {
	...
	public:
		...
		/**
		 * @brief Return a pointer to the name of table which stores column
		 *
		 *  Require definition of the SQLITE_ENABLE_COLUMN_METADATA preprocessor macro :
		 * - for compilation of the SQLite library,
		 * - and also when compiling this wrapper.
		 */
		const char* getTableName(void)const throw(); // nothrow

		/**
		 * @brief Return a pointer to the name of database which stores column
		 *
		 *  Require definition of the SQLITE_ENABLE_COLUMN_METADATA preprocessor macro :
		 * - for compilation of the SQLite library,
		 * - and also when compiling this wrapper.
		 */
		const char* Column::getDatabaseName(void)const throw();
