...
namespace SQLite {
	...
	mQuery(apQuery), mStmtPtr(aDatabase.mpSQLite, mQuery),
	...
	Statement::Statement(Database & aDatabase, const std::string apQuery) :
	// throw(SQLite::Exception)
	mQuery(apQuery), mStmtPtr(aDatabase.mpSQLite, mQuery),
	...
	// throw(SQLite::Exception)
	{
		int ret = sqlite3_bind_text(mStmtPtr, aIndex, aValue.c_str(),
			aValue.size(), SQLITE_TRANSIENT);
		check(ret);
	}
	...
	void Statement::bind(const int aIndex, const void* apValue,
		const int aSize) // throw(SQLite::Exception)
	{
	...
	{
		int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
		int ret = sqlite3_bind_text(mStmtPtr, index, aValue.c_str(),
			aValue.size(), SQLITE_TRANSIENT);
		check(ret);
	}
	...
	{
		int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
		int ret = sqlite3_bind_text(mStmtPtr, index, apValue, -1,
			SQLITE_TRANSIENT);
		check(ret);
	}

	// Bind a binary blob value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL prepared statement
	void Statement::bind(const char* apName, const void* apValue,
		const int aSize) // throw(SQLite::Exception)
	{
		int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
	 ...
	 */
	Statement::Ptr::Ptr(sqlite3* apSQLite, std::string& aQuery) : mpSQLite
		(apSQLite), mpStmt(NULL), mpRefCount(NULL) {
	...
	mpStmt(aPtr.mpStmt), mpRefCount(aPtr.mpRefCount) {
