...
namespace SQLite {
	...
	public:
		...
		/**
		 * @brief Creates uninitialized database instance. open() method must
		 * be called for opening database.
		 */
		Database(void);
		 ...
		 * @throw SQLite::Exception in case of error
		 */
		int exec(const std::string apQueries); // throw(SQLite::Exception);
		 ...
		 * @throw SQLite::Exception in case of error
		 */
		Column execAndGet(const std::string apQuery);
		// throw(SQLite::Exception);
